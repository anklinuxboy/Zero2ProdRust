...
impl Application {
    pub async fn build(configuration: Settings) -> Result<Self, anyhow::Error> {
        ...
        println!("hostname: {}", &configuration.application.host);
        let address = format!(
            "{}:{}",
...
pub async fn run(
    ...
    println!("RUN: after message store");
    let redis_store = RedisSessionStore::new(redis_uri.expose_secret()).await?;
    println!("RUN: after redis store");
    ...
    println!("RUN: after server started");
