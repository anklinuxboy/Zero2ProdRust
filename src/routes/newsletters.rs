...
use actix_web::{trace, web, HttpRequest, HttpResponse, ResponseError};
...
async fn validate_credentials(
    ...
    .await
    .context("Failed to spawn a task")
    .map_err(PublishError::UnexpectedError)??;
...
fn basic_authentication(headers: &HeaderMap) -> Result<Credentials, anyhow::Error> {
    ...
    let decoded_bytes = base64::decode_config(base64encoded_segment, base64::STANDARD)
        .context("Failed to based64-decode 'Basic' credentials")?;
    let decoded_credentials =
        String::from_utf8(decoded_bytes).context("The decoded credential is not valid utf-8")?;
